...
public class BlastDustedEffect extends MobEffect {
	...
	public void detonate(LivingEntity entity, int amp) {
		...
		entity.level.explode(player != null ? player : entity, (player != null) ? new EntityDamageSource("blastDust", player).setExplosion() : ADDamageSources.BLAST_DUST, null, entity.getX(), entity.getY() + (entity.getBbHeight() * 0.4), entity.getZ(), (float) BlastBreathAbility.getExplosionPower(amp), flag, flag ? Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.NONE);
		if (player != null)
		...
		if (localPlayer != null) {
			if (instance.getEffect() == ADDragonEffects.BLAST_DUSTED) {
				...
				for (int i = Functions.secondsToTicks(BlastBreathAbility.blastBreathEffectDuration); i > instance.getDuration(); i-= 20) {
					if (i % 100 == 20)
						localPlayer.getLevel().addAlwaysVisibleParticle(ParticleTypes.FLAME, entity.getX() + (random.nextFloat() * 2) - 1.0f, entity.getY() + (random.nextFloat() * entity.getEyeHeight()), entity.getZ() + (random.nextFloat() * 2) - 1.0f, 0.0, 0.0, 0.0);
					else
						localPlayer.getLevel().addAlwaysVisibleParticle(new LargeBlastDustParticleData(24, false, color), entity.getX() + (random.nextFloat() * 2) - 1.0f, entity.getY() + (random.nextFloat() * entity.getEyeHeight()), entity.getZ() + (random.nextFloat() * 2) - 1.0f, 0.0, 0.0, 0.0);
				}
			}
		}
	}
