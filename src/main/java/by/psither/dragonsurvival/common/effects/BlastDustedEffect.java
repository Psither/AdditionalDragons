...
public class BlastDustedEffect extends MobEffect {
	...
	public void detonate(LivingEntity entity, int amp) {
		if (entity.level().isClientSide())
			return;
		boolean flag = true;//entity.getMobGriefingEvent(entity.level(), entity);
	...
	public void detonate(Entity entity, int amp) {
		if (entity.level().isClientSide())
			return;
		boolean flag = true;//net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(entity.level(), entity);
	...
	public boolean applyEffectTick(LivingEntity entity, int amp) {
		if (entity.level().isClientSide()) {
			double timeLeft = (((float) entity.getEffect(ADDragonEffects.BLAST_DUSTED).getDuration() / 20.0F) / BlastBreathAbility.blastBreathEffectDuration);
			int color = BlastBreathAbility.getIntColorFromTimeLeft(timeLeft);
			RandomSource random = entity.getRandom();
			for (int i = Functions.secondsToTicks(BlastBreathAbility.blastBreathEffectDuration); i > entity.getEffect(ADDragonEffects.BLAST_DUSTED).getDuration(); i-= 20) {
				if (i % 60 == 20) {
					...
					entity.level().addAlwaysVisibleParticle(new LargeBlastDustParticle.Data(24, false, color), entity.getX() + (random.nextFloat() * 2) - 1.0f, entity.getY() + (random.nextFloat() * entity.getEyeHeight()), entity.getZ() + (random.nextFloat() * 2) - 1.0f, 0.0, 0.0, 0.0);
				}
			}
		}
		return true;
			...
			if (instance.getEffect() == ADDragonEffects.BLAST_DUSTED) {
				...
				for (int i = Functions.secondsToTicks(BlastBreathAbility.blastBreathEffectDuration); i > instance.getDuration(); i-= 20) {
					if (i % 100 == 20)
						...
						localPlayer.level().addAlwaysVisibleParticle(new LargeBlastDustParticle.Data(24, false, color), entity.getX() + (random.nextFloat() * 2) - 1.0f, entity.getY() + (random.nextFloat() * entity.getEyeHeight()), entity.getZ() + (random.nextFloat() * 2) - 1.0f, 0.0, 0.0, 0.0);
				}
			}
		}
	}
